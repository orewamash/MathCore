...
const ParticleCard: React.FC<{
  ...
  useEffect(() => {
    ...
    element.addEventListener('click', handleInternalClick);
    ...
    return () => {
      ...
      element.removeEventListener('click', handleInternalClick);
      clearAllParticles();
    };
...
const BentoCardGrid: React.FC<{
  ...
  <div
    className={`bento-section grid gap-2 p-3 w-full select-none relative ${className}`}
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  return (
    <>
      ...
      <BentoCardGrid gridRef={gridRef} className="card-responsive">
          {cards.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] w-full p-6 rounded-3xl border border-white/5 font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-1 ${
            ...
            return (
              ...
              >
                ...
                <div className="card__content flex flex-col relative text-white">
                  ...
                  <p
                    className={`card__description text-sm leading-relaxed text-white/60 font-bold italic ${textAutoHide ? 'text-clamp-2' : ''}`}
                  >
                    {card.description}
                  </p>
                </div>
